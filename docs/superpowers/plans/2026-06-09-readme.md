...
**Goal:** Create a production-ready `README.md` at the project root that serves as the single authoritative entry point for onboarding, knowledge transfer, and enterprise adoption of the Insurity EAIS Automation Framework.

**Architecture:** One comprehensive reference file (`README.md`) with a ToC, 13 self-contained sections, and one Mermaid architecture diagram. It links out to existing `SETUP.md` and `ONBOARDING.md` rather than duplicating their content.

**Tech Stack:** Markdown (GitHub Flavored), Mermaid diagrams, inline code blocks, badge shields.
...
|---|---|---|
| Create | `README.md` | Complete framework reference document |

---

## Task 1 — Header Block, Badges, Overview, and Table of Contents

**Files:**
- Create: `README.md`

- [ ] **Step 1: Create README.md with header block, badge row, elevator pitch, quick links, and ToC**
...
- [ ] **Step 2: Verify the file exists and the markdown header renders correctly**

Open `README.md` in a Markdown preview. The badge row should show 5 coloured badges. The ToC links should be present.

- [ ] **Step 3: Commit**

```bash
git add README.md
git commit -m "docs: add README header, badges, and table of contents"
```

---

## Task 2 — Framework Overview Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Framework Overview section**
...
```bash
git add README.md
git commit -m "docs: add Framework Overview section"
```

---

## Task 3 — Architecture Section (with Mermaid Diagram)
...
```mermaid
graph TD
    T["🧪 Tests<br/><code>tests/**/*.spec.ts</code>"]
    H["⚙️ Helpers / Workflows<br/><code>src/core/utils/*-helper.ts</code>"]
    SO["🔌 Service Objects<br/><code>src/api/clients/</code>"]
    PO["📄 Page Objects<br/><code>src/ui/pages/</code>"]
    FX["🔩 Fixtures<br/><code>src/*/fixtures/</code>"]
    U["🛠 Core Utils<br/><code>src/core/utils/</code>"]

    T -->|"orchestrate steps,<br/>assert outcomes"| H
    T -->|"direct API calls<br/>for simple specs"| SO
    T -->|"direct UI actions<br/>for simple specs"| PO
    H --> SO
    H --> PO
    SO --> FX
    PO --> FX
...
Open the README in a Mermaid-compatible viewer (GitHub, VS Code with Mermaid extension, or https://mermaid.live). The diagram should show 6 nodes with directional arrows.

- [ ] **Step 3: Commit**

```bash
git add README.md
git commit -m "docs: add Architecture section with Mermaid layer diagram"
```

---

## Task 4 — Folder Structure Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Folder Structure section**
...
```bash
git add README.md
git commit -m "docs: add Folder Structure section with path alias table"
```

---

## Task 5 — Getting Started Section
...
```bash
git add README.md
git commit -m "docs: add Configuration section with env var reference and encryption guide"
```

---

## Task 7 — Test Data Strategy Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Test Data Strategy section**
...
`tc-map-pod3-{suiteId}.json` files are hand-authored overrides that explicitly map a TC tag (e.g., `TC-AI-001`) to an ADO work item ID. Use these when the auto-resolution fails for a specific test.
...
```bash
git add README.md
git commit -m "docs: add Test Data Strategy section"
```

---

## Task 8 — Running Tests Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Running Tests section**
...
```bash
# By project name (defined in playwright.config.ts)
...
```bash
git add README.md
git commit -m "docs: add Running Tests section with npm scripts and project table"
```

---

## Task 9 — Reporting & Evidence Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Reporting & Evidence section**
...
```bash
git add README.md
git commit -m "docs: add Reporting and Evidence section"
```

---

## Task 10 — CI/CD Integration Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the CI/CD Integration section**
...
1. Copy the `Pod3_Rules` stage block
2. Change the `name`, `displayName`, and `npm run test:XXX` script
3. Update `dependsOn` in the `PushToADO` stage to include the new stage
4. Add any new service-specific env vars to `insurity-config`
...
```bash
git add README.md
git commit -m "docs: add CI/CD Integration section"
```

---

## Task 11 — ADO Integration Deep-Dive Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the ADO Integration section**
...
3. **Numeric parse** — if the test title contains `TC-17461`, the number is extracted directly
...
// At the top of a describe block
useSuiteAnnotations(18178, 123456); // (suiteId, planId)
```
...
Before first run:
  ...
  → Commit these files (CI uses them without ADO access)
...
When a test cannot be auto-resolved (new test with no matching title in ADO, or title changed), add an explicit entry:
...
Where the key is the `TC-AI-XXX` tag in the test title and the value is the ADO work item numeric ID.
...
```bash
git add README.md
git commit -m "docs: add ADO Integration Deep-Dive section"
```

---

## Task 12 — Best Practices Section
...
- Title starts with `Verify` — describes the observable outcome, not the implementation
...
| HTTP requests (`GET`, `POST`, etc.) | Service objects | Tests (directly, unless trivial), page objects |
| Multi-step reusable flows | Helpers | Tests (if used 2+ times), page objects |
...
test.describe.serial('LIFECYCLE — Rule CRUD', () => {
  test('Create rule', ...);      // state set here
...
Attach evidence to every API interaction for ADO publishing:
...
Use `description` (not `value`) — Playwright only shows `description` in the HTML report.
...
| Call `expect()` inside a page object | Page objects are state — assertions belong in tests |
...
```bash
git add README.md
git commit -m "docs: add Best Practices section"
```

---

## Task 13 — Scalability Recommendations Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Scalability Recommendations section**
...
5. **Add a pipeline stage** in `azure-pipelines.yml` (copy-paste the `Pod3_Rules` stage, update env vars)

### Adding a New Environment

1. Create `.env.{envname}` with the environment-specific overrides (DB host, URLs)
2. Add `npm run test:{envname}` to `package.json`: `"cross-env TEST_ENV={envname} playwright test"`
3. Add an `insurity-config-{envname}` variable group in Azure Pipelines Library
4. Add an optional pipeline stage or parameter to target the new environment
...
3. Add the role to `AUTH_STORAGE_PATH`: `newrole: '.auth/newrole.json'`
4. Add the authentication step in `global-setup.ts` (copy an existing role block)
5. Run `global-setup.ts` once locally to generate `.auth/newrole.json` — then re-run will be auto
...
// CI: 2 workers — conservative for shared agents
```

Increase `workers` on CI if your agent has more cores and the app under test can handle concurrent load. For suites with serial dependencies, `test.describe.serial()` forces sequential execution within that group regardless of the worker count.

For very large suites (100+ tests), use Playwright sharding:
...
Each shard runs independently and can be distributed across parallel CI agents.

### When to Refactor Serial → Parallel

Refactor a `test.describe.serial()` block to parallel when:
- Each test can create its own test data (use `TestDataFactory` per test, not shared state)
- The resource created in `beforeAll` can be scoped to `beforeEach` without significant cost
- Parallel execution would meaningfully reduce total run time
...
```bash
git add README.md
git commit -m "docs: add Scalability Recommendations section"
```

---

## Task 14 — Contributing Guidelines Section

**Files:**
- Modify: `README.md` (append)

- [ ] **Step 1: Append the Contributing Guidelines section**
...
- Test assertions: verify they test business outcomes, not implementation details (HTTP verbs, internal field names)
...
> **New to the codebase?** Start with [ONBOARDING.md](ONBOARDING.md) for a full guided walkthrough of the framework patterns, fixture system, and your first test end-to-end.
...
- [ ] **Step 2: Verify the full README renders correctly**

Open `README.md` in a Markdown preview (VS Code, GitHub, or Azure Repos). Verify:
- All 13 section headers are present and linked from the ToC
- The Mermaid diagram renders as a graph (not raw text)
- Code blocks have correct syntax highlighting
- Tables are properly formatted
- Badge images appear at the top

- [ ] **Step 3: Final commit**

```bash
git add README.md
git commit -m "docs: add Contributing Guidelines section — README complete"
```
...
|---|---|
| Framework overview | Task 2 (Section 1) |
| Architecture | Task 3 (Section 2) — Mermaid diagram + layer table |
| Folder structure | Task 4 (Section 3) — annotated tree + alias table |
| Test data strategy | Task 7 (Section 6) — three-layer model, StateManager, cleanup |
| Configuration | Task 6 (Section 5) — env var table, ENC: format, overlay |
| Execution commands | Task 8 (Section 7) — full npm scripts table + project table |
| Reporting | Task 9 (Section 8) — artifacts, trace viewer, ADO push, archival |
| CI/CD integration | Task 10 (Section 9) — stages, variable groups, triggers |
| ADO integration | Task 11 (Section 10) — TC resolution, annotations, workflow |
| Best practices | Task 12 (Section 11) — naming, layer table, never-do list |
| Scalability | Task 13 (Section 12) — new service, env, role, sharding |
| Contributing | Task 14 (Section 13) — branches, PR checklist, encryption |
| Complement SETUP.md / ONBOARDING.md | Getting Started (Task 5) + Contributing (Task 14) — callout boxes with links |
| Mixed audience tone | All sections — "why this exists" notes alongside technical details |
| Single architecture Mermaid diagram | Task 3 — `graph TD` layer diagram |

All 13 spec requirements have corresponding tasks. No gaps.

### Placeholder Scan

All code blocks contain actual content. No TBD, TODO, or "fill in later" language present.

### Type Consistency

No cross-task type references (this is a documentation plan, not a code plan). All command examples use the exact script names from `package.json`.
